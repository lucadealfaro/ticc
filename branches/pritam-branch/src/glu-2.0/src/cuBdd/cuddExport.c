...
#ifndef lint
static char rcsid[] DD_UNUSED = "$Id: cuddExport.c,v 1.1 2005/04/21 05:58:00 luca Exp $";
#endif
...
static int ddDoDumpDaVinci ARGS((DdManager *dd, DdNode *f, FILE *fp, st_table *visited, char **names, unsigned long mask));
static int ddDoDumpDDcal ARGS((DdManager *dd, DdNode *f, FILE *fp, st_table *visited, char **names, unsigned long mask));
  ...
  FILE * fp /* pointer to the dump file */)
{
    ...
    */
	...
	retval = fprintf(fp, "n%lx%s\n",
			 ((unsigned long) f[i] & mask) / sizeof(DdNode),
