/**CHeaderFile*****************************************************************
  ...
  Revision    [$Id: cuddInt.h,v 1.1 2005/04/21 05:58:00 luca Exp $]
...
EXTERN DdNode * cuddBddNPAndRecur ARGS((DdManager *dd, DdNode *f, DdNode *c));
...
EXTERN int cuddLinearInPlace ARGS((DdManager * table, int  x, int  y));
EXTERN void cuddUpdateInteractionMatrix ARGS((DdManager * table, int  xindex, int  yindex));
EXTERN int cuddInitLinear ARGS((DdManager *table));
EXTERN int cuddResizeLinear ARGS((DdManager *table));
...
#endif
EXTERN DdNodePtr * cuddNodeArray ARGS((DdNode *f, int *n));
