...
(** The function takes a list of symbolic modules and returns a list of cloned
    symbolic modules. For each module m with name "m.name", a new module is 
    created with name m.name+"_clone".   
*)
let symbolic_clone (ml:Symmod.t list):Symmod.t list =

  let clone l m = 
    (Prog.clone_modp Prog.toplevel m.name ((m.name)^"_clone"));    
    let sm = mk_sym ((m.name)^"_clone") in 
    sm::l in
  List.fold_left clone [] ml
