...
#ifndef lint
static char rcsid[] DD_UNUSED = "$Id: cuddEssent.c,v 1.1 2005/04/21 05:58:00 luca Exp $";
#endif
...
static DdTlcInfo * ddFindTwoLiteralClausesRecur ARGS((DdManager * dd, DdNode * f, st_table *table));
static DdTlcInfo * computeClauses ARGS((DdTlcInfo *Tres, DdTlcInfo *Eres, DdHalfWord label, int size));
static DdTlcInfo * computeClausesWithUniverse ARGS((DdTlcInfo *Cres, DdHalfWord label, short phase, int size));
static DdTlcInfo * emptyClauseSet ARGS((void));
static int sentinelp ARGS((DdHalfWord var1, DdHalfWord var2));
static int equalp ARGS((DdHalfWord var1a, short phase1a, DdHalfWord var1b, short phase1b, DdHalfWord var2a, short phase2a, DdHalfWord var2b, short phase2b));
static int beforep ARGS((DdHalfWord var1a, short phase1a, DdHalfWord var1b, short phase1b, DdHalfWord var2a, short phase2a, DdHalfWord var2b, short phase2b));
static int oneliteralp ARGS((DdHalfWord var));
static int impliedp ARGS((DdHalfWord var1, short phase1, DdHalfWord var2, short phase2, BitVector *olv, BitVector *olp));
static BitVector * bitVectorAlloc ARGS((int size));
DD_INLINE static void bitVectorClear ARGS((BitVector *vector, int size));
static void bitVectorFree ARGS((BitVector *vector));
DD_INLINE static short bitVectorRead ARGS((BitVector *vector, int i));
DD_INLINE static void bitVectorSet ARGS((BitVector * vector, int i, short val));
static DdTlcInfo * tlcInfoAlloc ARGS((void));
  ...
  DdNode * f)
{
    ...
    st_foreach_item(table, gen, (char **)&node, (char **)&tlc) {
	if (node != f) {
...
/**Function********************************************************************
  ...
  SeeAlso     [Cudd_FindTowLiteralClauses]
  ...
  FILE *fp)
{
    ...
    BitVector *phases;
    int i;
    DdTlcInfo *res = Cudd_FindTowLiteralClauses(dd, f);
...
/**Function********************************************************************
  ...
  Synopsis    [Implements the recursive step of Cudd_FindTowLiteralClauses.]

  Description [Implements the recursive step of
  Cudd_FindTowLiteralClauses.  The DD node is assumed to be not
