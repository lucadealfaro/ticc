...
#ifndef lint
static char rcsid[] DD_UNUSED = "$Id: cuddUtil.c,v 1.5 2006/01/28 05:31:32 vishwa Exp $";
#endif
...
static int cuddNodeArrayRecur ARGS((DdNode *f, DdNodePtr *table, int index));
...
Cudd_bddPickOneMintermRandom(
  ...
  DdNode ** vars /* array of variables */,
  int  n /* size of <code>vars</code> */,
  int overAllVars /* whether we need all variables assigned */)
{
    char *string;
    int i, size;
    int *indices;
    ...
    /* Randomize choice for don't cares. This is how we make sure that
       we get complete minterms during simulation. If we do not pick
       the values of unspecified variables at random, we will have
       minterms that cover all possible choices of the variables missing
       in the random cube */
